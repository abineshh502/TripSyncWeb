...
import { 
  MapPin, Route, Navigation2, Compass, Loader2, 
  ChevronRight, ArrowLeft, ExternalLink, Play, Square, 
  Map, Calendar, Clock, Navigation
} from "lucide-react";
...
// OSRM & GPS Constants
const REROUTE_THRESHOLD_KM = 0.15; // 150m
...
// Helper for perpendicular distance
...
export default function SharedRoutePage() {
  ...
  // Fetch steps instructions turn-by-turn guidance from OSRM
  const fetchOSRMSteps = async (startLat: number, startLon: number, destinations: MapSpot[]) => {
    ...
    try {
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        // Update route coordinates to match active tracking
  ...
  const startNavigation = () => {
    ...
    // Set initial target zoom to focus on user
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        // Fetch fresh turn-by-turn navigation route from user current location
